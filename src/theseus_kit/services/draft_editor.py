...
class DraftEditor:
    ...
    *robot_id* is stored for interface symmetry with :class:`ConfigReader`
    ...
    async def update_draft(
        ...
        *,
        ...
        if expected_hash is not None:
            current = await self._get_current_draft(client, path)
    ...
    @staticmethod
    async def _get_current_draft(client: RobotClient, path: str) -> dict[str, Any]:
        """Fetch the current draft DTO; raises typed errors on failure."""
        try:
            response = await client.get(path)
        except RobotApiError as exc:
            if exc.status_code == 404:
                raise DraftNotFoundError(f"Draft at {path} not found (404).") from exc
            raise

        body: dict[str, Any] = response.json()
        code: int = body.get("code", 0)
        if code == 404:
            raise DraftNotFoundError(f"Draft at {path} not found (404).")
        if code != 200:
            raise RobotApiError(
